...
export function getBrowserInfo(appVersion: string, userAgent: string): IBrowserInfo {
	...
	let matches = CHROMIUM_EDGE_VERSION.exec(appVersion);
	if (matches != null) {
		return createBrowserInfo(BrowserType.Edge, matches);
	}

	matches = CHROME_VERSION.exec(appVersion);
	if (matches != null) {
...
export const browserInfo = getBrowserInfo(window.navigator.appVersion, window.navigator.userAgent);
