...
function wrapConsoleLogFunction(f: ConsoleLogFunctionsList): ConsoleLogFunction {
	...
	try {
		if (SUPPORTS_BIND &&
			window.console[f] != null &&
			window.console[f].bind != null) {

			return window.console[f].bind(console) as ConsoleLogFunction;
		}
	} catch (e) {
	}

	return window.console[f];
